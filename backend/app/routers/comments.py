...
@router.post("/comment", response_model = schemas.CommentInDB, status_code = status.HTTP_201_CREATED)
async def create_comment(
    ...
    comment_in = schemas.CommentCreate(paper_id = paper_id, reviewer_id = reviewer_id, ipfs_hash = ipfs_hash)
    ...
    if not reviewer : raise HTTPException(status_code = 404, detail = "Not found reviewer")
    user = db.query(models.User).filter(models.User.id == reviewer.user_id).first()
    if not user : raise HTTPException(status_code = 404, detail = "Not found user")
    ...
    try :
        ...
        db_comment = crud.update_paper_tx_hash(db, db_comment.id, tx_hash)

    except Exception as e : raise HTTPException(status_code = 500, detail = f"등록 실패 :: {str(e)}")
...
def list_comments_of_paper(paper_id : int, db : Session = Depends(get_database)) :
    comments = crud.get_comments_of_paper(db, paper_id)
    return comments
